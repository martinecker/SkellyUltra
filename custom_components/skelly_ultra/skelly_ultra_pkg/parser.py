...
def parse_notification(sender: Any, data: bytes) -> Optional[Union[LightInfo, VolumeEvent, LiveNameEvent, DeviceParamsEvent, WifiMacEvent, StartTransferEvent, ChunkDroppedEvent, TransferEndEvent, ResumeWriteEvent, PlayPauseEvent, DeleteFileEvent, FormatEvent, CapacityEvent, MusicOrderEvent, FileInfoEvent]]:
    ...
    if hexstr.startswith("BBE1"):
        ...
        for i in range(6):
            ...
            lights.append(LightInfo(channel=channel, brightness=brightness, rgb=(r, g, b), chEffect=chEffect, effectGroup=effectGroup))
        eye_icon = int(hexstr[90:92], 16)
        return FileInfoEvent(file_index=0, cluster=0, total_files=0, length=0, attr=action, eye_icon=eye_icon, db_pos=0, name="", lights=lights)
    ...
    if hexstr.startswith("BBE0"):
        ...
        return DeviceParamsEvent(channels=channels, pin_code=pin_code, wifi_password=wifi_password, display_mode=display_mode, name=name)
    ...
    if hexstr.startswith("BBD2"):
        ...
        return CapacityEvent(capacity_kb=capacity, file_count=file_count, mode_str=mode_str)
    ...
    if hexstr.startswith("BBD0"):
        ...
        try:
            ...
            name = ""
        return FileInfoEvent(file_index=file_index, cluster=cluster, total_files=total_files, length=length, attr=attr, eye_icon=eye_icon, db_pos=db_pos, name=name, lights=lights)
