...
# Query Commands
def query_device_parameter() -> bytes: return build_cmd("E0")
def query_live_mode() -> bytes: return build_cmd("E1")
def query_volume() -> bytes: return build_cmd("E5")
def query_live_name() -> bytes: return build_cmd("E6")
def query_version() -> bytes: return build_cmd("EE")
def query_file_info() -> bytes: return build_cmd("D0")
def query_song_order() -> bytes: return build_cmd("D1")
def query_capacity() -> bytes: return build_cmd("D2")

# Media Controls
def set_volume(vol: int) -> bytes: return build_cmd("FA", int_to_hex(vol, 1))
def play() -> bytes: return build_cmd("FC", "01")
def pause() -> bytes: return build_cmd("FC", "00")
def enable_classic_bt() -> bytes: return build_cmd("FD", "01")
def set_music_mode(mode: int) -> bytes: return build_cmd("FE", int_to_hex(mode, 1))

# RGB & Light Controls
def set_mode(channel: int, mode: int, cluster: int, name: str) -> bytes:
...
def set_brightness(channel: int, brightness: int, cluster: int, name: str) -> bytes:
    ch = "FF" if channel == -1 else int_to_hex(channel, 1)
    name_utf16 = to_utf16le_hex(name)
    name_len = int_to_hex((len(name_utf16) // 2) + 2, 1) if name else "00"
...
def set_rgb(channel: int, r: int, g: int, b: int, loop: int, cluster: int, name: str) -> bytes:
    ch = "FF" if channel == -1 else int_to_hex(channel, 1)
    name_utf16 = to_utf16le_hex(name)
    name_len = int_to_hex((len(name_utf16) // 2) + 2, 1) if name else "00"
...
def set_speed(channel: int, speed: int, cluster: int, name: str) -> bytes:
    ch = "FF" if channel == -1 else int_to_hex(channel, 1)
    name_utf16 = to_utf16le_hex(name)
    name_len = int_to_hex((len(name_utf16) // 2) + 2, 1) if name else "00"
...
def start_send_data(size: int, max_pack: int, filename: str) -> bytes:
    return build_cmd("C0", int_to_hex(size, 4) + int_to_hex(max_pack, 2) + "5C55" + to_utf16le_hex(filename))
